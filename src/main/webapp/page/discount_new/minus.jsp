	...
	<script type="text/javascript">
		  $(function(){
			  ...
			  $("input[name=radio]").on("click",function(){
				  if($(this).val() == 1){
					  $(".random_lj").removeClass("show");
					  $(".fixed_lj").removeClass("hide").addClass("show");
				  }else{
					  $(".fixed_lj").removeClass("show").addClass("hide");
					  $(".random_lj").addClass("show");
				  }
				  $("#sum_ys").val("");
			  });
...
// 				  alert('添加');
              ...
              //添加随机立减
              $("#add_random_rule").on("click",function(){
            	 var randomRule = '<div class="rulegroup">'
			           +'<div class="rule_ys clear">'
			           +'<label>预算</label>'
			           +'<div class="input">'
			           +'<input type="number" class="form-control random_ys_lj_money" >'
			           +'</div>'
			           +'<div class="rule_font_m">'
			           +'<span>元</span>'
			           +'</div>'
			           ...
			           +'<div class="rule_consume_row">'
			           +'<span>消费满</span>'
			           +'<input type="number" class="form-control money consume_m"/>'
			           +'<span>元</span>'
			           +'<div class="count">'
			           +'<span>名额</span>'
			           +'<input type="number" class="form-control money me">'
			           +'<span>次</span>'
			           +'</div>'
			           +'</div>'
			           +'</div>'
			           +'<div class="rule_minus_interval">'
			           +'<div class="rule_consume_row qj">'
			           +'<span>减价区间</span>'
			           +'<input type="number" class="form-control money "/>'
			           +'<span>至</span>'
			           +'<input type="number" class="form-control money "/>'
			           ...
			           $(".random_rule").append(randomRule);
			           $(".random_ys_lj_money").on("blur",function(){
			            	  cursor_random_leave();
			           });
			           $(".qj").find("input").on("blur",function(){
			            	  dynamic_qj(this);
			           });
              });
              
              $("#del_random_rule").on("click",function(){
            	  var rules = $(".random_rule").find(".rulegroup"),len=rules.length;
            	  if(len==1){
            		  alert("只有一条了不能被删除。");
            		  return false;
            	  }else{
              ...
              //随机函数
              function cursor_random_leave(){
            	  var moneys = $(".random_ys_lj_money"),
        	      moneylen = moneys.length,
        	      sum = 0;
	        	  for(var i = 0;i<moneylen;i++){
	        		  sum += Number($(moneys[i]).val());
              ...
              //动态区间监听
              function dynamic_qj(obj){
            	  var other = $(obj).siblings("input");
            	  if(other.val() != "" && $(obj).val() != ""){
            		  var sum = Number(other.val())+Number($(obj).val());
            		  $(obj).siblings(".average").find("input").val(sum/2);
            		  var ys = $(obj).parents(".rulegroup").find(".random_ys_lj_money").val();
            		  $(obj).parents(".rulegroup").find(".me").val(Math.floor(Number(ys)/(sum/2)));
            	  }
              };
              ...
              $("#oneNext").on("click",function(){
            	  ...
            	  var ljstyle = $("input[name=radio]:checked").val();
            	  var ruleList = [];
            	  if(ljstyle == 1){//满立减
            		  ...
            		  var randomrules = $(".random_rule").find(".rulegroup"),len = randomrules.length;
            	      for(var i = 0;i < len;i ++){
            	    	  var rule = {};
            	    	  var inputs = $(randomrules[i]).find("input"),inputlen = inputs.length;
            	    	  for(var j = 0;j < inputlen;j ++){
            	    		  if($(inputs[j]).val() == ""){
            	    			  alert("规则不能有空值");
            	    			  return false;
            	  ...
            	  ljInfo.type = $("input[name=radio]:checked").val();
            	    ...
            	    if(ljInfo.type == 1){//满立减
            	    	...
            	    	for(var i =0,len=rules.length;i<len;i++){
                            ...
                            +'     </div>'
                                ...
                                +'     <label>消费满</label>'
                                +'     <div>'
                                +'       <span>'+rules[i].f1+'</span>元，立减'
                                +'       <span>'+rules[i].f2+'</span>元'
                                +'     </div>'
                                +' </div>'
                                        +' </div>'
                                        +'</div>';
            	    	...
            	    	for(var i =0,len=rules.length;i<len;i++){
            	    		var s = '<div class="end_rule clear">'
                                ...
                                +'   <div class="endrule_row clear">'
                                    ...
                                    +'      <label>消费满</label>'
                                         +'       <div>'
                                         +'         <span>'+rules[i].f1+'</span>元'
                                         +'      </div>'
                                         +'   </div>'
                                      +'   <div class="row">'
                                      +'    <label>名额</label>'
                                      +'     <div>'
                                      +'     <span>'+rules[i].f2+'</span>'
                                            +'  </div>'
                                            +' </div>'
                                            +'</div>'
                                            +' <div class="endrule_row clear">'
                                            ...
                                            +'    <label>减价区间</label>'
                                            +'    <div>'
                                            +'    <span>'+rules[i].f3+'</span>元至<span>'+rules[i].f4+'</span>元'
                                            +'  </div>'
                                            +' </div>'
                                            +' <div class="row">'
                                            +'   <label>均值</label>'
                                            +'   <div>'
                                            +'    <span>'+rules[i].f5+'</span>元'
                                            +'  </div>'
                                            +'</div>'
                                            +'  </div>'
                                            +' </div>'
...
//             	  alert("创建成功"); 
//             	  ljInfo.payStyle = (ljInfo.payStyle).toString();
//             	  ljInfo.rules = JSON.stringify(ljInfo.rules);
//             	  ljInfo.storeNos = (ljInfo.storeNos).toString();
//             	  ljInfo.name = $("#minusName").val();
//             	  ljInfo.type = $("input[name=radio]:checked").val();
//             	  ljInfo.startTime = $("#startTime").val();
//             	  ljInfo.endTime = $("#endTime").val();
                     ...
                     <div class="detail">
                        <div class="baseinfo">
                           <div class="form-horizontal ajaxfrom" role="form">
							  ...
							  <div class="form-group">
									<label class="col-xs-12 col-sm-2 col-md-2 col-lg-2 control-label">优惠方式</label>
									<div class="col-sm-10 col-lg-9 col-xs-12">
										<label class="radio-inline"><input type="radio" name="radio" value="1" checked> 定额立减</label>
										<label class="radio-inline"><input type="radio" name="radio" value="2" > 随机立减</label>
									</div>
							 </div>
							 <div class="form-group">
			                 ...
			                 <div class="form-group">
							  ...
							  <div class="fixed_lj show">
								 <div class="ruleList">
							            <div class="fixed_minus_rule">固定减价规则</div>
							  ...
							  <div class="random_lj hide">
								 <div class="ruleList">
							            <div class="random_minus_rule">随机减价规则</div>
								        <div class="random_rule">
								           <div class="rulegroup">
									           <div class="rule_ys clear">
									             <label>预算</label>
									             <div class="input">
								        ...
								        <div class="rule_btns">
									               <span id="add_random_rule">新增规则</span>
									               <span id="del_random_rule">删除规则</span>
									    </div>
								 </div>
							  </div>
                              ...
                              <div class="usestyle_content">
                                 ...
                                 <div class="jt_operate">
                                    ...
                                    <div class="jtgroup clear">
                                      ...
                                      <div class="jtright">
                                         <label>
                                         <input type="checkbox" name="paystyle" value="1" checked disabled>
                                                                           微信支付</label>
                                         <label>
                                         <input type="checkbox" name="paystyle" value="2" checked disabled>支付宝支付</label>
                                      </div>
                                    </div>
                                    <div class="jtgroup clear">
                                      <label>同时使用</label>
