	...
	<script type="text/javascript">
	   $(function(){
		  ...
		  $.get("module/getModules",function(result){
			  ...
			  //用于查询该用户组已经关联的模块
			  $.post("module/getModulesByUserGroupId",{
				  userGroupId:<%=id%>
			  },function(result){
				 if(result.code == 0){
				    var obj = result.data,
				    ...
				    for(var i in obj){
					    for(var j in input){
					    	if(obj[i].id == $(input[j]).val() && 
					    			obj[i].parentModule.id == $(input[j]).parent().parent().prev().prev().val() &&
					    			obj[i].parentModule.parentModule.id == $(input[j]).parent().parent().parent().parent().prev().prev().val()
					    			){
					    		$(input[j]).prop("checked",true);
					    		$(input[j]).parent().parent().prev().prev().prop("checked","true");
					    		$(input[j]).parent().parent().parent().parent().prev().prev().prop("checked","true");
					    		break;
					    	}
		  ...
		  $("#submit").on("click",function(){
			     ...
			     $.post("userGroup/addUserGroupModules",{
			    	 userGroupId:<%=id%>,
