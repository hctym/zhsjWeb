	...
	*{
	...
	<script type="text/javascript">
	   $(function(){
		   ...
		   $.post("shop/getList?page=1&pageSize=100",function(result){
			   if(result.code == 0){
				   var tbody = $("tbody");
				   for(var i in result.data){
					   if(result.data[i].ctime >0 ){
						   result.data[i].ctime =  new Date(result.data[i].ctime).Format("yyyy-MM-dd");
					   }
					   tbody.append($("<tr>")
							   .append($("<td>").text(result.data[i].id))
							   .append($("<td>").text(result.data[i].contact))
							   .append($("<td>").text(result.data[i].contact))
							   .append($("<td>").text(result.data[i].contactPhone))
							   .append($("<td>").text(result.data[i].shopName))
							   ...
							   .append($("<td>").append($("<span>").attr("class","label label-success").
								   ...
								   alert("edit  "+$(this).attr("data-id"));
									   ...
									   .append($("<span style='margin-left:10px;'>").attr("class","label label-info")
											   .attr("data-id",result.data[i].id).text("删除").on("click",function(){
												   alert("delete  "+$(this).attr("data-id"));
											   })).append($("<span style='margin-left:10px;'>").attr("class","label label-info")
													   .attr("data-id",result.data[i].id).text("添加用户").on("click",function(){
														   window.location.href="page/addShopUser?type=1&id="+$(this).attr("data-id");
													   })).append($("<span style='margin-left:10px;'>").attr("class","label label-info")
															   .attr("data-id",result.data[i].id).text("查看商家用户").on("click",function(){
		 														     window.location.href ="page/shopUsers?id="+$(this).attr("data-id");
															   }))));
				   }
			   }else{
				   alert(result.msg);
